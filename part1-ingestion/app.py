...
store = JobStore()
fetcher = FeedFetcher()
SOURCE_URL = os.getenv("JOB_FEED_URL", "mock://local/jobs")
...
def ingest_from_source() -> IngestResponse:
    ...
    source = SOURCE_URL
    try:
        xml_text = read_source(SOURCE_URL)
        raw_jobs = parse_rss(xml_text)
        if not raw_jobs:
            if store.count():
                logger.warning("Source returned zero jobs; preserving %s existing jobs", store.count())
                return IngestResponse(source=source, jobs_seen=0, jobs_added=0, used_fallback=False, message="Empty response ignored")
            raise ValueError("Source returned zero jobs")
    except Exception as error:
        logger.warning("Source unavailable (%s); using controlled mock feed", error)
    ...
    logger.info("Ingested %s jobs, added %s new jobs", len(jobs), added)
    return IngestResponse(source=source, jobs_seen=len(jobs), jobs_added=added, used_fallback=used_fallback, message="Ingestion completed")
